...
| 43 | Runtime-configurable temp curve | Replace compile-time `TEMP_CURVE_POINTS` and hand-rolled `voltageToCelsius()` with a `CurveInterpolator` (same as tank level). Web UI calibration table, persisted to flash. Seed with VDO Type A defaults. Curve input changes from raw voltage to voltage ratio if #42 is done first | Medium |
